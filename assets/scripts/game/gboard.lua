...
local update_tutorial =  require "game.tutorial"
...
local function updtut(board)
  local n = 0
  for _,v in ipairs(board.items) do
    if v.tag == board.current_level.tag then
      n=n+1
      if v.state.pin or v.state.arrow then
        n=n+1
      end
    end
  end
  if n >= board.current_level.ntotal then
    update_tutorial(board)
  end
end
...
local function key_down(a,b)
  ...
  if code==49 then
     ...
     upd_tutorial =  require "dev"
     if upd_tutorial then
       update_tutorial = upd_tutorial
       update_tutorial(board)
     end
  end

  if code==50 then
     package.loaded["dev-level"]=false
     local _lvl = require "dev-level"
     if _lvl then
       board:_setLevel(level(_lvl))
     end
  end
